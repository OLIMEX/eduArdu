...
OliLedMatrix::OliLedMatrix(int latchPin, int dataPin, int clockPin)
{
...
*/
void OliLedMatrix::drawArray(int x, int y, const uchar *array, int rows)
{
	for (int i=0; i<rows; i++)
...
// draws a pixel
void OliLedMatrix::drawPixel(int x, int y)
{
...
// erase a Pixel
void OliLedMatrix::erasePixel(int x, int y)
{
	if ((x<0) || (x>=ROWS) || (y<0) || (y>=ROWS))
		return;
...
void OliLedMatrix::drawLine(int x, int y, int length, boolean horizontal)
{
	if (!horizontal)
...
// draws a rectangle
void OliLedMatrix::drawRectangle(int x, int y, int w, int h, boolean filled)
{
	if (filled)
	{
		for(int i=y; i<y+h; i++)
